...
public class TransformerController extends RestController {
	...
	static String objectName = "transformer";
	...
	protected Object post(Request request, Response response) throws Exception {
		...
		for (int index = 0, size = jsonArray.size(); index < size; index++) {
			...
			// 温度
			Float template = jsonObj.getFloat("template");
			// 负载率
			Float load_rate = jsonObj.getFloat("load_rate");
			// 更新数据时间

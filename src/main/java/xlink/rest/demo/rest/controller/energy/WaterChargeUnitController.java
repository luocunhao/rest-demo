...
public class WaterChargeUnitController extends RestController {

	static String serviceId = "energy_management";
	static String objectName = "water_charge_unit";
	...
	protected Object post(Request request, Response response) throws Exception {
		...
		for (int index = 0, size = jsonArray.size(); index < size; index++) {
			...
			// 设备用水计费_月
			Float group_charge_month = jsonObj.getFloat("group_charge_month");
			// 设备用水计费_年
			Float group_charge_year = jsonObj.getFloat("group_charge_year");
			// 添加数据时间
			...
			data.put("group_charge_month", group_charge_month);
			data.put("group_charge_year", group_charge_year);
			data.put("date", date);

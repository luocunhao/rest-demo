...
public class PowerCompensationController extends RestController {

	private static final Logger logger = LoggerFactory.getLogger(PowerCompensationController.class);

	static String serviceId = "power_distribution_system";
	static String objectName = "h_volatge_power_compensation";
	...
	protected Object post(Request request, Response response) throws Exception {
		...
		for (int index = 0, size = jsonArray.size(); index < size; index++) {
			...
			// 无功补偿电流
			Float svc_electric = jsonObj.getFloat("svc_electric");
			// 无功功率
			...
			data.put("svc_electric", svc_electric);
